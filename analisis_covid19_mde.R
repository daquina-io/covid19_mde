...
## Acumulados MDE
acumuladosMde <- plot_ly(  x = mde_infectados_df$date, y = mde_infectados_df$total, type ='bar', name='Medellin' )%>%
...
porDiaAnt <- plot_ly(  x = compare_data$date, y = compare_data$antAcumulado, type ='bar', name= 'Antioquia',color = I("darkolivegreen") )%>%
...
acumCol <- plot_ly(  x = infectados_df$date, y = infectados_df$total, type ='bar', color = I("darkOliveGreen") )%>%
